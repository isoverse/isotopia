...
test_that("Testing that basic single data types' (ratio, abundance, delta, etc.) validity controls are working", {
    ...
    expect_equal(label(epsilon(-10)), paste(get_iso_letter("epsilon"), "[‰]"))
    expect_equal(label(epsilon(`13C` = -0.01, permil = FALSE)), paste("13C", get_iso_letter("epsilon")))
    expect_equal(label(epsilon(`13C` = -10, ctop = "SO4", cbot = "H2S")), paste0("13C ", get_iso_letter("epsilon"), "_SO4/H2S [‰]"))
    expect_warning({
    ...
    expect_equal(label(delta(-10)), paste(get_iso_letter("delta"), "[‰]"))
    expect_equal(label(delta(`13C` = -0.01, permil = FALSE)), paste0(get_iso_letter("delta"),"13C"))
    expect_equal(label(delta(`13C` = -10, compound = "DIC", ref = "SMOW")), paste0("DIC ", get_iso_letter("delta"), "13C [‰] vs. SMOW"))
    expect_warning({
        d <- delta(25, ref = "SMOW")
        d <- delta(d, ref = "Air")
