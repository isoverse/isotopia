...
test_that("Testing that basic single data types' (ratio, abundance, delta, etc.) validity controls are working", {
    ...
    expect_equal(label(alpha(0.9)), get_iso_letter("alpha"))
    expect_equal(label(alpha(`13C` = 0.9)), paste("13C", get_iso_letter("alpha")))
    expect_equal(label(alpha(`34S` = 0.9, ctop = "SO4", cbot = "H2S")), paste0("34S ", get_iso_letter("alpha"), "_SO4/H2S"))
    expect_warning({
    ...
    expect_equal(label(epsilon(-10)), paste0(get_iso_letter("epsilon"), " [", get_iso_letter("permil"), "]"))
    expect_equal(label(epsilon(`13C` = -0.01, permil = FALSE)), paste0("13C ", get_iso_letter("epsilon")))
    expect_equal(label(epsilon(`13C` = -10, ctop = "SO4", cbot = "H2S")), paste0("13C ", get_iso_letter("epsilon"), "_SO4/H2S [", get_iso_letter("permil"), "]"))
    expect_warning({
    ...
    expect_equal(label(delta(-10)), paste0(get_iso_letter("delta"), " [", get_iso_letter("permil"), "]"))
    expect_equal(label(delta(`13C` = -0.01, permil = FALSE)), paste0(get_iso_letter("delta"),"13C"))
    expect_equal(label(delta(`13C` = -10, compound = "DIC", ref = "SMOW")), paste0("DIC ", get_iso_letter("delta"), "13C [", get_iso_letter("permil"), "] vs. SMOW"))
    expect_warning({
        d <- delta(25, ref = "SMOW")
        d <- delta(d, ref = "Air")
